...
const createProductSchema = z.object({
  ...
  description: z.string().min(10, { message: "Product description must be at least 50 characters."}),
...
export default function CreateProductForm() {
  ...
  const { fields, append, remove } = useFieldArray<z.infer<typeof createProductSchema>>(
    {
  ...
  async function onSubmit(values: z.infer<typeof createProductSchema>){
    ...
    const res = await createProduct(values.name, values.description, Number(values.price), values.category, Number(values.stock),pictures)
        ...
        {fields.map((field, index) => (
          <div className="flex flex-col space-y-4" key={index}>
            <div key={field.id} className='flex flex-row space-x-4'> 
              <Input placeholder="Enter picture links" {...createProductForm.register(`pictures.${index}.url` as const, { required: true })} />
